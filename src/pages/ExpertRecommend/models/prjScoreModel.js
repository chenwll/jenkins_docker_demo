...
export default {
  ...
  effects: {
    *getRule({ payload }, { call, put, select }) {
      const { scoreId, type,reviewYear} = payload;
      const stateTmp = yield select(_ => _.prjScoreModel);
      const { api } = stateTmp;
      const response = yield call(api[type].score.getRule, {scoreId,reviewYear});
      ...
      if(response.code === 0) {
        ...
        const { prjScore: ruleTree, state, score, grade, projectName,suggestion,saveState} = response.data;
