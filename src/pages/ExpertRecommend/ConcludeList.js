...
import { PROJECT_PROCESS_TYPE, RECOMMEND_STATE,SCORE_RECOMMEND,PROJECT_IMPLEMENTATION_PLAN,RECOMMEND_SCORE ,RECOMMEND_TYPE} from '../../utils/Enum';
...
class  ExpertRecommend extends PureComponent{
  ...
  listPage=(params)=>{
    ...
    const {dispatch,expertRecommend:{ConcludePagination}}=this.props;
    ...
    ConcludePagination.currentPage===0?ConcludePagination.currentPage=1:ConcludePagination.currentPage;
    dispatch({
      type:`expertRecommend/${url}`,
      payload:{
        data:isParamsUndefined.length !== 0 ? {...params,guideStage:2 ,
          reviewYear: date}: {  ...ConcludePagination,
          guideStage: 2,
          reviewYear: date},
        type:'conclude'
      }
  ...
  handleChooseGuide=(record)=>{
    ...
    if(type) {
      ...
      router.push(`/expertApproval/projectApproval/step1/0/${record.projectId}/${record.scoreId}/${record.reviewYear}/${record.processId}/${RECOMMEND_TYPE.CONCLUDE}`);
      // dispatch({
  ...
  render(){
    ...
    const {expertRecommend:{GuideSelectedRows, departList, guideList, detail,ConcludeGuideList}}=this.props;
    ...
    return(
      <PageHeaderWrapper title="结项专家项目评分">
        <Card bordered={false}>
