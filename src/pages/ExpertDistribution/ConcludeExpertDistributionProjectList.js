...
import { Form, Card, Button, message,Modal,Select } from 'antd';
...
import ConcludeExpertDistribution from './ConcludeExpertDistribution'
...
componentWillMount(){
  ...
  dispatch({
    type: 'expertDistributionModel/recommendType',
    payload: {
      recommendType:'conclude'
    }
  });
}
  ...
  listPage = (params) => {
    ...
    const {searchParams,year}=this.state;
    dispatch({
  ...
  renderExpert = () => {
    ...
    return {
      ...
      drawerContent: <ConcludeExpertDistribution {...contentOptionsRecommend} />,
    };
  };
  ...
  handleFormReset = () => {
    const { form,dispatch,match : { params: { type, ...pageInfo } } } = this.props;
    form.resetFields();
    this.setState({
      year:pageInfo.reviewYear
    })
    dispatch({
      type: 'expertDistributionModel/getRecommendDetail',
      payload: {
    ...
    searchList=searchList.concat(
      [{
        ...
        renderComponent : () => {
          ...
          for(let i=0;i<=10;i++){
            yearArr.push(+nowYear+1+i);
          };
         yearArr.sort();
          return  <Select onChange={this.YearChange}>{yearArr.map(year=><Option id={`${year}`} key={year}>{year}</Option>)}</Select>
        }

      }
      ]
    );
    const cardTitle = params.type==='1'?'非高校结项阶段':'高校结项阶段';
    const columns = [
      {
        title:'项目名称',
        ...
        <Card bordered={false} title={<h2>{cardTitle}</h2>}>
          <div className={styles.tableList}>
            ...
            <StandardTable
              ...
              rowKey={record => String(record.projectId)+String(record.allocated)}
            />
          </div>
